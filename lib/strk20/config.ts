...
export const STARKNET_EXPLORER_TRANSACTION_URL = "https://voyager.online/tx";
...
export function getStrk20Config(): Strk20Config | null {
  ...
  if (
    ...
    || poolAddress.toLowerCase() !== STRK20_POOL_ADDRESS.toLowerCase()
  ) return null;
...
export function getStarknetExplorerTransactionUrl(hash: string): string {
  return `${STARKNET_EXPLORER_TRANSACTION_URL}/${encodeURIComponent(hash)}`;
}
